    ...
    RSpec.describe EgressRules do
      ...
      describe '#staging' do
        ...
        context 'when the app space has staging security groups with the same rule as default staging' do
          before do
            security_group = SecurityGroup.make(guid: 'guid7', rules: [{ 'protocol' => 'udp', 'ports' => '8080-9090', 'destination' => '198.41.191.47/1' }], staging_default: false)
            security_group.add_staging_space(space)
          end
      ...
      describe '#running' do
        ...
        it 'should provide the egress rules in the start message' do
          expect(egress_rules.running(process)).to match_array([
            { 'protocol' => 'udp', 'ports' => [8080], 'destinations' => ['198.41.191.47/1'] , 'annotations' => ['security_group_id:guid1']},
            { 'protocol' => 'tcp', 'port_range' => { 'start' => 9090, 'end' => 9095 }, 'destinations' => ['198.41.191.48/1'], 'log' => true, 'annotations' => ['security_group_id:guid2'] },
            { 'protocol' => 'udp', 'ports' => [1010, 2020], 'destinations' => ['198.41.191.49/1'], 'annotations' => ['security_group_id:guid3'] },
          ])
        end
